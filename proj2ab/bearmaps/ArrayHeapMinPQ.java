...
public class ArrayHeapMinPQ<T> implements ExtrinsicMinPQ<T>{
    ...
    public T removeSmallest() {
        ...
        itemMap.remove(minPQ.get(1).val);
        size = size - 1;
        sink(1);
        return returnValue;
    }
    ...
    private void sink(int index) {
        ...
        if (minPQ.get(leftChild).priority < minPQ.get(index).priority) {
            exchange(minPQ.get(leftChild), minPQ.get(index));
            itemMap.put(minPQ.get(index).val, index);
            itemMap.put(minPQ.get(leftChild).val, leftChild);
            swim(leftChild);
        }
    }
    @Override
